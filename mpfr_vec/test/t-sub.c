...
main(void)
{
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        c = _mpfr_vec_init(length, prec);
        _mpfr_vec_randtest(a, state, length);
        _mpfr_vec_randtest(b, state, length);

        _mpfr_vec_sub(c, a, b, length);
        _mpfr_vec_sub(a, a, b, length);

        result = (_mpfr_vec_equal(a, c, length));
        if (!result)
        {
            printf("FAIL:\n");
            _mpfr_vec_print(a, length, 10), printf("\n\n");
            _mpfr_vec_print(c, length, 10), printf("\n\n");
            abort();
        }

        _mpfr_vec_clear(a, length);
        _mpfr_vec_clear(b, length);
        _mpfr_vec_clear(c, length);
    }
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        _mpfr_vec_sub(c, a, b, length);
        _mpfr_vec_sub(b, a, b, length);

        result = (_mpfr_vec_equal(b, c, length));
        if (!result)
        {
            printf("FAIL:\n");
