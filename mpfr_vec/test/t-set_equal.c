...
main(void)
{
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        _mpfr_vec_set(a, a, length);

        result = (_mpfr_vec_equal(a, a, length));
        if (!result)
        {
            printf("FAIL:\n");
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        _mpfr_vec_set(b, a, length);
        _mpfr_vec_set(b, b, length);
    ...
    {
        mpfr *a, *b;
        slong coeff, length = n_randint(state, 100) + 1;
        mpfr_prec_t prec = n_randint(state, 200) + MPFR_PREC_MIN;
